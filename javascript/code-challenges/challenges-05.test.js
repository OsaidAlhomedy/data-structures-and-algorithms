...
const calculateAverage = (arr) => {
  ...
  const avg = accumulator.sum / accumulator.count;
  return avg;
};
...
const extractStat = (statName, arr) => {
  const newObj = arr.reduce((acc, n) => {
    if (n.stat.name == statName) {
      acc = n;
    }
    return acc;
  }, 0);
...
const extractChildren = (arr) => {
  const newArr = arr
    .filter((item) => (item.name.includes("a") ? true : false))
    .reduce((acc, item) => {
      if (item.children) {
